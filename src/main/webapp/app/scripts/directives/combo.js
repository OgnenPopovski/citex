            ...
            controller: function($scope, $element, crudService, $injector) {
                ...
                $scope.formatResult = function(item) {
                    ...
                    if (groupProp && item[groupProp]) {
                        ...
                        markedText = item[property] + ' - <b>' + x[groupNameProp] + '</b>';
                    }
                ...
                if ($scope.comboFormatResult && typeof $scope.comboFormatResult === 'function') {
                ...
                $scope.matcher = function(term, text, item) {
                    if (groupProp && item[groupProp]) {
                        return item[groupNameProp].toUpperCase().indexOf(
                            term.toUpperCase()) >= 0 || item[groupProp][groupNameProp].toUpperCase().indexOf(
                            term.toUpperCase()) >= 0;
                    } else {
                ...
                $scope.reload = function() {
                    var populate = function(data) {
                        ...
                        if ($scope.options.data.results && $scope.options.data.results.length == 1 && $scope.autoSet) {
...
Auction.directive('cmbChange', function() {
    return {
        restrict: 'A',
        link: function(scope, element, attrs) {
            element.bind('change', function(e) {
                var fn = scope.$eval(attrs.cmbChange);
